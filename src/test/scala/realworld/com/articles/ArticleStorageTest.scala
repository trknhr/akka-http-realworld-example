...
class ArticleStorageTest extends BaseServiceTest{
    ...
    "getArticles" should {
      "return article by author id" in new Context {
        awaitForResult(for {
          ...
          articles <- articleStorage.getArticles(ArticleRequest(authorName = Some("author"), tag = None, favorited = None, limit= 10, offset= 0))
  ...
  trait Context {
    val articleStorage: ArticleStorage = new JdbcArticleStorage(InMemoryPostgresStorage.databaseConnector)
    val userStorage: UserStorage = new JdbcUserStorage(InMemoryPostgresStorage.databaseConnector)

    val testArticle1 = Article(0, "title-one", "tile-one", "test description", "test body", 1, createdAt = currentWhenInserting, updatedAt = currentWhenInserting)
    val author = User(1, "author", "test","test", None, image = None, createdAt = currentWhenInserting, updatedAt = currentWhenInserting)
