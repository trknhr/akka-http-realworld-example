  ...
  "ProfileService" when {
    "getProfile" should {
      "return profile by id" in new Context {
        (userStorage.getUserByUsername _)
          .expects("username-1") returning DBIO.successful { Some(testUser1) }
        (userStorage.isFollowing _).expects(*, *) returning DBIO.successful { true }
    ...
    "follow" should {
      "return profile with follow true" in new Context {
        (userStorage.getUserByUsername _)
          .expects("username-1") returning DBIO.successful { Some(testUser1) }
        (userStorage.follow _).expects(*, *) returning DBIO.successful { 1 }
    ...
    "unfollow" should {
      "return profile with follow false" in new Context {
        (userStorage.getUserByUsername _)
          .expects("username-1") returning DBIO.successful { Some(testUser1) }
        (userStorage.unfollow _).expects(*, *) returning DBIO.successful { 1 }
    ...
    "getFollowees" should {
      "return profiles" in new Context {
        (userStorage.getFollowees _).expects(*) returning DBIO.successful {
          Seq(testUser1)
        }
        awaitForResult(for {
