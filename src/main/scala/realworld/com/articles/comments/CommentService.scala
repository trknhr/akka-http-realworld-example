      ...
      (for {
        ...
        CommentResponse(
          CommentData(c.id,
                      ...
                      c.body,
                      u.username,
                      Profile(u.username, u.bio, u.image, follow)))).dbio)

  def getComments(slug: String, userId: Long): Future[CommentsResponse] =
    runner.run(
      for {
        a <- articleStorage.getArticleBySlug(slug)
        comments <- commentStorage.getComments(a.map(b => b.id).getOrElse(-1L))
