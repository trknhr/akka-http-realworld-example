...
package object core {
  ...
  case class UserWithToken(username: String, email: String, bio: Option[String], image: Option[String], token: AuthToken)

  case class UserUpdate(username: Option[String], password: Option[String], email: Option[String], bio: Option[String], image: Option[String]) {
    def merge(user: User): User = {
      User(
        user.id
        , username.getOrElse(user.username)
        , password.flatMap(s => Some(s.sha256.hex)).getOrElse(user.password.sha256)
  ...
  case class UserRegistration(username: String, password: String, email: String) {
    def create(): User = {
      User(0, username, password.sha256.hex, email, None, None, new Timestamp((new Date).getTime), new Timestamp((new Date).getTime))
    }
  }
}
